...
class DqnAgent:
    ...
    def save(self, msg=""):
        ...
        torch.save(q_state, "./models/dqn_q_model" + msg + ".pt")
        target_state = {
                'state_dict': self.target.state_dict(),
                'optimizer': self.optimizer.state_dict(),
        }
        torch.save(target_state, "./models/dqn_target_model" + msg + ".pt")

    def load(self, msg=""):
        checkpoint_q = torch.load("./models/dqn_q_model" + msg + ".pt")
        self.q.load_state_dict(checkpoint_q['state_dict'])
        self.optimizer.load_state_dict(checkpoint_q['optimizer'])
        self.q.train()
        checkpoint_target = torch.load("./models/dqn_target_model" + msg + ".pt")
